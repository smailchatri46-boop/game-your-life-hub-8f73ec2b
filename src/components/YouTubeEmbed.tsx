...
export function YouTubeEmbed({
    ...
    return (
      <div className={`aspect-video ${className}`}>
        <iframe
          ...
          allowFullScreen
          className="block h-full w-full rounded-t-xl"
          style={{ border: "none" }}
        />
      </div>
    );
  ...
  return (
    <div
      className={`aspect-video relative cursor-pointer group overflow-hidden ${className}`}
      onClick={handlePlay}
    >
      <img
        src={thumbnail || `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`}
        alt="Video thumbnail"
        className={`block w-full h-full object-cover origin-center ${thumbnailClassName}`}
        loading="lazy"
        onError={(e) => {
      ...
      {showThumbnailBottomFade && (
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-foreground/60 via-foreground/20 to-transparent" />
      )}
      ...
      <div className="absolute inset-0 bg-foreground/0 group-hover:bg-foreground/5 transition-colors duration-300 flex items-center justify-center">
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, hsl(25 95% 53%) 0%, hsl(35 100% 55%) 50%, hsl(25 95% 50%) 100%)",
            boxShadow:
              "0 8px 32px rgba(245, 130, 32, 0.4), 0 4px 16px rgba(245, 130, 32, 0.3)",
          }}
        >
          <Play
            className="w-7 h-7 text-white ml-1 drop-shadow-sm transition-transform duration-300 group-hover:scale-105"
            fill="white"
          />
        </div>
      </div>
